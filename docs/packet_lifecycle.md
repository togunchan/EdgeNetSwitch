...
must not be used as the correctness key because it may be duplicated or
...
- `duplicate_events` counts repeated terminal events for the same `lifecycle_id`
...
Replay validation uses the same lifecycle contract.

- `ReplayRecorder` stores ordered `PacketRx` ingress records.
- `ReplayPlayer` republishes recorded ingress into the normal runtime path.
- `ReplayOutcomeCollector` records ordered terminal events.

The replay stream remains ingress-only. Terminal outcomes are not replayed from
the recording; they must be regenerated by admission, failure injection,
validation, processing, and drop attribution. A replay is equivalent only when
the terminal observable history matches by order, terminal type,
`lifecycle_id`, and drop reason.
