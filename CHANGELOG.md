...
### Added
- Added `ReplayRecord`, `ReplayRecorder`, and `ReplayPlayer` as the replay infrastructure for ordered packet ingress capture and replay.
- Added `ReplayOutcome` and `ReplayOutcomeCollector` for observable terminal history capture across processed and dropped packet outcomes.
- Added lifecycle-keyed deterministic failure replay through `FailureConfig::lifecycle_rules`.
- Added replay outcome equivalence tests that compare original and replayed terminal histories.

### Changed
- Defined the replay stream as ingress-only: replay records store `PacketRx` input, while terminal outcomes are regenerated by the runtime under the same deterministic policy.
- Preserved the identity boundary where `packet.id` remains payload identity and `lifecycle_id` remains runtime-owned execution identity.
- Extended deterministic validation from aggregate `PacketStats` equivalence to terminal observable equivalence.

### Validated
- Verified replay equivalence for ordering, lifecycle ordering, drop attribution, and terminal observable history.
- Verified deterministic terminal event ordering through `ReplayOutcomeCollector` sequence numbers.
- Verified reproducible failure replay under lifecycle-based simulated-loss rules.
- Verified replay convergence across processed packets, validation drops, injected drops, and lifecycle-preserving terminal events.

### Engineering Notes
- Replay correctness is proven by comparing observable terminal histories, not by trusting the replay input stream alone.
- The ingress-only replay model keeps recorded data narrow and requires the runtime to reproduce its own outcomes during validation.
- Lifecycle-keyed failure rules make fault replay stable even when payload packet IDs are duplicated or externally controlled.
- This release completes the replay-verifiable deterministic runtime contract established by lifecycle-based accounting and deterministic failure injection.
